...
export default function AuthForm({ onSuccess }) {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      if (typeof signInWithGoogle === 'function') {
        ...
        throw new Error('Google sign-in not implemented (check lib/firebaseAuth).');
      }
    } catch (error) {
  ...
  return (
    <div className="auth-card" role="dialog" aria-labelledby="auth-title">
      ...
      <p style={{ marginTop: 0, marginBottom: 12, color: 'var(--muted, #bdbdbd)', fontSize: 13 }}>
        {mode === 'signin'
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginTop: 8 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <input
              ...
              disabled={busy}
            />
            <span style={{ fontSize: 13, color: 'var(--muted, #bdbdbd)' }}>Remember me</span>
          </label>
      ...
      <div style={{ marginTop: 14 }}>
        <div style={{ textAlign: 'center', fontSize: 13, color: 'var(--muted, #bdbdbd)', marginBottom: 8 }}>or</div>
        <button
          type="button"
      ...
      {err && (
        <div role="alert" className="error" aria-live="polite" style={{ marginTop: 12 }}>
          {err}
        </div>
      )}
